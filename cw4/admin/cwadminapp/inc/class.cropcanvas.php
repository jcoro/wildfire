...
{ 
	...
	function addFrame($file,$w,$h)
	{
		$frame_details = getimagesize($file);
		if($frame_details[2] == '1'){
			# MERGE FRAME IMAGE (note: for now, frame images MUST be gif format)
			$frameimage = imagecreatefromGIF($file);
			imagecopymerge($this->_imgFinal, $frameimage, 0,0,0,0,$w,$h,99);	
		} else if($frame_details[2] == '3'){
			# MERGE FRAME IMAGE (PNG format frame)
			$frameimage = imagecreatefromPNG($file);
			imagecopymerge($this->_imgFinal, $frameimage, 0,0,0,0,$w,$h,99);
		}
	}
    ...
    { 
        ...
        if ($type == 'gif') $saved = $func($this->_imgFinal, $filename);
    ...
    function cropToDimensions($sx, $sy, $ex, $ey, $position = ccCENTRE) 
    { 
        $nx = abs($ex - $sx); 
        $ny = abs($ey - $sy); 
    ...
    {
	return @ImageSX($this->_imgOrig);
    }

    function getOrigHeight()
    {
	return @ImageSY($this->_imgOrig);
    }
    ...
    { 
        ...
        ImageCopyResampled($this->_imgFinal, $this->_imgOrig, 
				    0, 0, $ox, $oy, 
				    $nx, $ny, $nx, $ny); 
    } 
